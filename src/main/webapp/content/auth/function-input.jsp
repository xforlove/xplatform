			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<form class="form-horizontal" action="/${scopeUrl }/app/function-save.do" method="POST" role="form">
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<input type="text" class="form-control" id="action" name="action" value="${model.action }">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-4">
							<select id="groupId" name="groupId" class="singleselect" multiple="multiple">
								<c:forEach items="${funcGrps }" var="object" varStatus="status">
									<option value="${object.id }" ${object.id == model.group.id ? 'selected' : ' '}>${object.name }</option>
								</c:forEach>
				            </select>
						</div>
					</div>
